	...
	init: function(editor) {
			...
			exec: function(editor) {
				...
				if (emailsubj.length > 255) {
					var ckerr = new CKEDITOR.plugins.notification(editor, {
						message: 'Email Subject is too long and will be truncated. Review it before sending the email.',
						type: 'warning'
					});
					ckerr.show();
				}
				...
				if (emailbcc) {
					if (emailbcc.split(";").length > 495) {
						...
						ckerr = new CKEDITOR.plugins.notification(editor, {
							message: 'Too many BCC addresses to auto-populate. Please paste the email, then copy/paste the BCC list.',
							type: 'warning'
						});
						ckerr.show();
		...
		function recordEmail(editor) {
			...
			if (!rid) {
				recordStatus.update({
					type: 'warning',
					message: 'Unable to record email contents in CSI QuickBase record. Please do so manually.'
				});
				return;
			}
			...
			body = body.split(/\<td id\=\"body\"[^\>]+>/);
			if (body) {
				body = body[1].split(/<\/td\>/);
				body = body[0];
			}
			else {
				recordStatus.update({
				...
				body = "<p>Body of email not logged.</p>";
			}
			var url = "";
		...
		function updateResponsesSafeMode(editor, dateFid) {
			...
			if (!dateFid) {
				recordStatus.update({
					type: 'warning',
					message: 'Failed to update one or more records in CSI Email Tracker Quickbase. Please do so manually.'
				});
			...
			ridlist = sessionStorage.getItem("ridlist").toString().split(',');
					...
					.done(function(xml) {
						if ($(xml).find("errcode").text() == 0) {
							...
							if (completion < 1) {
								...
								recordStatus.update({
									type: 'success',
									message: 'Finished updating CSI Email Tracker QuickBase.'
								});
							}
						}
						else {
							...
							console.log("CKEditor Error: Email Tracker QuickBase returned error. " + errcode + ": " + errtext);
							badupdate++;
							completion = (goodupdate + badupdate) / ridlist.length;
							if (completion < 1) {
								recordStatus.update({
									progress: completion
								});
								...
								recordStatus.update({
									type: 'warning',
									message: 'Failed to update ' + badupdate + ' of ' + ridlist.length + ' records in CSI Email Tracker Quickbase. Please do so manually.'
								});
							}
						}
					})
		...
		function updateResponses(editor, dateFid) {
			//var editor = CKEDITOR.instances.editor
			if (editor.config.emailConfig.bccQB.safeMode == 1) {
				updateResponsesSafeMode(editor, dateFid);
				return;
			}
			var recordStatus = editor.showNotification('Updating CSI Email Tracker QuickBase - DO NOT CLOSE THIS WINDOW', 'progress', 0);
			...
			if (!dateFid) {
				recordStatus.update({
					type: 'warning',
					message: 'Unable to update CSI Email Tracker Quickbase. Please do so manually.'
				});
				console.log("CKEditor Error: No dateFid parameter supplied to updateResponses()");
				return false;
			}
				...
				.done(function(xml) {
					if ($('errcode', xml).text() == 0) {
						...
						if (toupdate != input) {
							recordStatus.update({
								type: 'warning',
								message: 'WARNING: CSI Email Tracker QuickBase did not return the expected response! Please verify record updates and notify the QuickBase\'s administrator.'
							});
						...
						if (added != 0) {
							recordStatus.update({
								type: 'warning',
								message: 'WARNING: CSI Email Tracker QuickBase did not return the expected response! Unwanted records were created! Please verify record updates and notify the QuickBase\'s administrator.'
							});
						...
						if (toupdate != updated) {
							...
							recordStatus.update({
								type: 'Success',
								message: 'Successfully updated ' + updated + ' CSI Email Tracker QuickBase records.'
							});
						}
					}
					else {
